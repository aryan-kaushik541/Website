...
import { useSendEmailPasswordMutation } from '../services/userAuthApi';

const ResetPassword = () => {
   
    const [sendEmailPassword,{isLoading}] = useSendEmailPasswordMutation()
    ...
    const handleSubmit = async (event) => {
        ...
        if (form.email) {
            if (!/\S+\.\S+/.test(form.email)) {
                ...
                const response = await sendEmailPassword(form);
            ...
            setMsg({ status: true, severity: 'warning', msg: 'Email Is Required' })
        }
    }
    return (
        <>
            ...
            <motion.div
                    ...
                    >
                <h2 className='md:hidden text-center mt-7 font-semibold text-xl'>Enter Your Register Email id</h2>
               
                    <form className='flex flex-col gap-6' onSubmit={handleSubmit} id="forgotpin">
                        <h2 className='text-center mt-7 font-semibold hidden md:block text-xl'>Enter Your Register Email id</h2>
